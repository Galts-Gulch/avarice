# OKCoin api
# https://www.okcoin.cn/t-1000097.html
# https://www.okcoin.cn/t-1000052.html
...
class MarketData(object):
    ...
    def ticker(self, symbol='btc_cny'):
        ...
        if(symbol == 'ltc_btc'):
            ...
            ltc_btc_bid = round(
                float(ltc_data['ticker']['buy']) / float(btc_data['ticker']['sell']), 8)
            ltc_btc_ask = round(
                float(ltc_data['ticker']['sell']) / float(btc_data['ticker']['buy']), 8)
    ...
    def get_history(self, symbol='btc_cny', since=None):
        ...
        history_url = 'https://www.okcoin.cn/api/trades.do?symbol='
    ...
    def get_closingtrades(self, n, N, symbol='btc_cny'):
        ...
        for j in range(0, N):
            ...
            while True:
                ...
                if i < -len(passtrade):
                    passtrade = self.get_history(
                        symbol=symbol, since=oldesttid - int(len(passtrade) * 1.9))
                    newotid = passtrade[0]['tid']
                    while oldesttid > passtrade[-1]['tid']:
                        passtrade = self.get_history(
                            symbol=symbol, since=newotid + 2)
                        newotid = passtrade[0]['tid']
                    oldesttid = newotid
                    i = -1
                # print ' i =', i ####debug
